...
``h2_on_dust`` (int)
    - Flag to enable H\ :sub:`2` formation on dust grains, dust cooling, and dust-gas heat transfer follow Omukai (2000).  This assumes that the dust to gas ratio scales with the metallicity.  **This is not extensively tested and should probably not be used for now.** Default: 0.
...
``three_body_rate`` (int)
    Flag to control which three-body H\ :sub:`2` formation rate is used.  0: Abel, Bryan & Norman (2002), 1: PSS83, 2: CW83, 3: FH07, 4: G08.  These are discussed in Turk et. al. (2011).  Default: 0.

``cie_cooling`` (int)
    Flag to enable H\ :sub:`2` collision-induced emission cooling from Ripamonti & Abel (2004).  Default: 0.

``h2_optical_depth_approximation`` (int)
    Flag to enable H\ :sub:`2` cooling attenuation from Ripamonti & Abel (2004).  Default: 0.
