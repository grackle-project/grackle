...
)
{
  ...
  grackle::impl::View<gr_float***> de(my_fields->internal_energy, my_fields->grid_dimension[0], my_fields->grid_dimension[1], my_fields->grid_dimension[2]);
  ...
  dclPar[1-1] = (my_rates->cloudy_metal.grid_parameters[0][my_rates->cloudy_metal.grid_dimension[1-1]-1] - my_rates->cloudy_metal.grid_parameters[0][1-1]) /
       (double)(my_rates->cloudy_metal.grid_dimension[1-1] - 1 );
  if (my_rates->cloudy_metal.grid_rank > 1)  {
    dclPar[2-1] = (my_rates->cloudy_metal.grid_parameters[1][my_rates->cloudy_metal.grid_dimension[2-1]-1] - my_rates->cloudy_metal.grid_parameters[1][1-1]) /
         (double)(my_rates->cloudy_metal.grid_dimension[2-1] - 1 );
  }
  if (my_rates->cloudy_metal.grid_rank > 2)  {
    dclPar[3-1] = (my_rates->cloudy_metal.grid_parameters[2][my_rates->cloudy_metal.grid_dimension[3-1]-1] - my_rates->cloudy_metal.grid_parameters[2][1-1]) /
         (double)(my_rates->cloudy_metal.grid_dimension[3-1] - 1 );
  }
  if (my_rates->cloudy_metal.grid_rank > 3)  {
    dclPar[4-1] = (my_rates->cloudy_metal.grid_parameters[3][my_rates->cloudy_metal.grid_dimension[4-1]-1] - my_rates->cloudy_metal.grid_parameters[3][1-1]) /
         (double)(my_rates->cloudy_metal.grid_dimension[4-1] - 1 );
  }
  if (my_rates->cloudy_metal.grid_rank > 4)  {
    dclPar[5-1] = (my_rates->cloudy_metal.grid_parameters[4][my_rates->cloudy_metal.grid_dimension[5-1]-1] - my_rates->cloudy_metal.grid_parameters[4][1-1]) /
    ...
    if ( itmask[i] != MASK_FALSE )  {
      ...
      if (my_rates->cloudy_metal.grid_rank == 1)  {
        log_cool[i] = grackle::impl::fortran_wrapper::interpolate_1d_g(
            ...
            dclPar[1-1], my_rates->cloudy_metal.data_size, my_rates->cloudy_metal.cooling_data);
        ...
        if (my_chemistry->UVbackground == 1)  {
            log_heat[i] = grackle::impl::fortran_wrapper::interpolate_1d_g(
              log10tem[i], my_rates->cloudy_metal.grid_dimension, my_rates->cloudy_metal.grid_parameters[0],
              dclPar[1-1], my_rates->cloudy_metal.data_size, my_rates->cloudy_metal.heating_data);
        ...
        log_cool[i] = grackle::impl::fortran_wrapper::interpolate_2d_g(
            ...
            my_rates->cloudy_metal.grid_parameters[0], dclPar[1-1], my_rates->cloudy_metal.grid_parameters[1], dclPar[2-1],
        ...
        if (my_chemistry->UVbackground == 1)  {
          log_heat[i] = grackle::impl::fortran_wrapper::interpolate_2d_g(
              log_n_h[i], log10tem[i], my_rates->cloudy_metal.grid_dimension,
              my_rates->cloudy_metal.grid_parameters[0], dclPar[1-1], my_rates->cloudy_metal.grid_parameters[1], dclPar[2-1],
              my_rates->cloudy_metal.data_size, my_rates->cloudy_metal.heating_data);
          edot_met[i] = edot_met[i] + std::pow(10.,log_heat[i]);
        }
        ...
        log_cool[i] = grackle::impl::fortran_wrapper::interpolate_4d_g(
            ...
            my_rates->cloudy_metal.grid_parameters[0], dclPar[1-1], my_rates->cloudy_metal.grid_parameters[1], dclPar[2-1],
            my_rates->cloudy_metal.grid_parameters[2], dclPar[3-1], my_rates->cloudy_metal.grid_parameters[3], dclPar[4-1],
            my_rates->cloudy_metal.data_size, my_rates->cloudy_metal.cooling_data);
        edot_met[i] = std::pow(-10.,log_cool[i]);
      ...
      edot[i] = edot[i] + (edot_met[i] * rhoH[i] * d(i,idx_range.j,idx_range.k));
