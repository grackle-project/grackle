...
void grackle::impl::calc_tdust_1d_g(
  ...
  double* isrf, gr_mask_type* itmask, double trad, int in, int gr_N,
  double* gr_dT, double* gr_Td, gr_float* alsp_data_, double* kgr,
  int* idspecies, IndexRange idx_range
)
{
  // opacity table of a grain species
  //
  // In some configurations gr_N can be 0 while the backing buffer may still be non-null.
  // The View invariant disallows non-null data with a zero leading extent,
  // so pass nullptr for the zero-length case.
  ...
  trad  = std::fmax(1., trad); // TODO: do we really want to write in a passed by copy input par?
  ...
  for (iter = 1; iter<=(itmax); iter++) {
    ...
    for (i = idx_range.i_start; i<=idx_range.i_end; i++) {
      if ( nm_itmask[i] != MASK_FALSE )  {
        ...
        tdplus[i] = std::fmax(1.e-3, ((1. + pert[i])
        * tdustnow[i]));
    ...
    // Calculate grain opacities
     ...
     FORTRAN_NAME(calc_kappa_gr_g)(tdustnow.data(), kgr, nm_itmask.data(),
        &in, &idx_range.i_start, &idx_range.i_end, &t_subl,
        &Td_N, &Td_Size, gr_dT, gr_Td, logalsp.data(), idspecies);

     FORTRAN_NAME(calc_kappa_gr_g)(tdplus.data(), kgrplus.data(), nm_itmask.data(),
        &in, &idx_range.i_start, &idx_range.i_end, &t_subl,
        &Td_N, &Td_Size, gr_dT, gr_Td, logalsp.data(), idspecies);
    ...
    // Calculate heating/cooling balance
     ...
     FORTRAN_NAME(calc_gr_balance_g)(tdustnow.data(), tgas, kgr, &trad4, gasgr,
         gamma_isrf.data(), nh, nm_itmask.data(), sol.data(), &in, &idx_range.i_start, &idx_range.i_end);

     FORTRAN_NAME(calc_gr_balance_g)(tdplus.data(), tgas, kgrplus.data(), &trad4, gasgr,
         gamma_isrf.data(), nh, nm_itmask.data(), solplus.data(), &in, &idx_range.i_start, &idx_range.i_end);
    ...
    for (i = idx_range.i_start; i<=idx_range.i_end; i++) {
      if ( nm_itmask[i] != MASK_FALSE )  {
        ...
        tdustnow[i] = std::fmin(tdustnow[i] - (sol[i] / slope[i]),
                         3e3);

        pert[i] = std::fmax(std::fmin(pert[i],
             (0.5 * std::fabs(tdustnow[i] - tdustold[i]) /
             tdustnow[i])), minpert);
  ...
  if (c_done < c_total)  {
    ...
    for (iter = 1; iter<=(bi_itmax); iter++) {
      ...
      FORTRAN_NAME(calc_gr_balance_g)(bi_t_mid.data(), tgas, kgr, &trad4, gasgr,
          gamma_isrf.data(), nh, bi_itmask.data(), sol.data(), &in, &idx_range.i_start, &idx_range.i_end);
      ...
      for (i = idx_range.i_start; i<=idx_range.i_end; i++) {
        if ( bi_itmask[i] != MASK_FALSE )  {
          ...
          if ((std::fabs(bi_t_high[i] - tdustnow[i]) / tdustnow[i])
               <= bi_tol)  {
            bi_itmask[i] = MASK_FALSE;
            c_done = c_done + 1;
          }
    ...
    if (iter > itmax)  {
      OMP_PRAGMA_CRITICAL
      {
        eprintf("CALC_TDUST_1D_G failed to converge after %d iterations for %d cells.\n",
                iter, (c_total - c_done));
      }
    }
  ...
  for (i = idx_range.i_start; i<=idx_range.i_end; i++) {
    if ( itmask[i] != MASK_FALSE )  {
      ...
      if (tdustnow[i] < 0.)  {
        OMP_PRAGMA_CRITICAL
        {
          eprintf("CALC_TDUST_1D_G Newton method -  T_dust < 0: i =  %d j =  %d k =  %d nh =  %g t_gas =  %g t_rad =  %g t_dust =  %g\n",
                  i,
                  idx_range.jp1,
                  idx_range.kp1,
                  nh [ i ],
