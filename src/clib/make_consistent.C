  ...
  //_// PORT:             SN_metal(:,12) = metal_Y19(:,j,k)
          ...
          for (i = my_fields->grid_start[0]; i<=my_fields->grid_end[0]; i++) {
            ...
            for (iSN = 1; iSN<=(nSN); iSN++) {
              ...
              Ct[i] =  Ct[i] + my_rates->SN0_XC [iSN0-1] * SN_metal(i,iSN-1);
              Ot[i] =  Ot[i] + my_rates->SN0_XO [iSN0-1] * SN_metal(i,iSN-1);
              Mgt[i] = Mgt[i] + my_rates->SN0_XMg[iSN0-1] * SN_metal(i,iSN-1);
              Alt[i] = Alt[i] + my_rates->SN0_XAl[iSN0-1] * SN_metal(i,iSN-1);
              Sit[i] = Sit[i] + my_rates->SN0_XSi[iSN0-1] * SN_metal(i,iSN-1);
              St[i] =  St[i] + my_rates->SN0_XS [iSN0-1] * SN_metal(i,iSN-1);
              Fet[i] = Fet[i] + my_rates->SN0_XFe[iSN0-1] * SN_metal(i,iSN-1);

              Cg[i] =  Cg[i] + my_rates->SN0_fC [iSN0-1] * SN_metal(i,iSN-1);
              Og[i] =  Og[i] + my_rates->SN0_fO [iSN0-1] * SN_metal(i,iSN-1);
              Mgg[i] = Mgg[i] + my_rates->SN0_fMg[iSN0-1] * SN_metal(i,iSN-1);
              Alg[i] = Alg[i] + my_rates->SN0_fAl[iSN0-1] * SN_metal(i,iSN-1);
              Sig[i] = Sig[i] + my_rates->SN0_fSi[iSN0-1] * SN_metal(i,iSN-1);
              Sg[i] =  Sg[i] + my_rates->SN0_fS [iSN0-1] * SN_metal(i,iSN-1);
              Feg[i] = Feg[i] + my_rates->SN0_fFe[iSN0-1] * SN_metal(i,iSN-1);
            }
          }
      ...
      for (i = my_fields->grid_start[0]; i<=my_fields->grid_end[0]; i++) {
        correctH = (gr_float)(my_chemistry->HydrogenFractionByMass*metalfree[i]/totalH[i] ); // TODO: which one is correct?
        // correctH = (gr_float)(my_chemistry->HydrogenFractionByMass*(1. - my_chemistry->DeuteriumToHydrogenRatio)*metalfree[i]/totalH[i]
        //              );
        // // ! GC202005
      ...
      if (my_chemistry->primordial_chemistry > 2)  {
        for (i = my_fields->grid_start[0]; i<=my_fields->grid_end[0]; i++) {
          ...
          correctD = (gr_float)(my_chemistry->HydrogenFractionByMass*my_chemistry->DeuteriumToHydrogenRatio*metalfree[i]/totalD );
      ...
      for (i = my_fields->grid_start[0]; i<=my_fields->grid_end[0]; i++) {
        ...
        if (my_chemistry->primordial_chemistry > 1) { de(i,j,k) = de(i,j,k)
             - HM(i,j,k) + H2II(i,j,k)/(gr_float)(2.); }
        if (my_chemistry->primordial_chemistry > 3) { de(i,j,k) = de(i,j,k)
             - DM   (i,j,k)/(gr_float)(2.)
