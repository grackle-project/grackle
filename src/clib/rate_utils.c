...
typedef struct {double * data; const char* name;} rateprop_;
...
enum MiscRxnRateKind_ {
  MiscRxn_k13dd,
  MiscRxn_k24, MiscRxn_k25, MiscRxn_k26,
  MiscRxn_k27, MiscRxn_k28, MiscRxn_k29, MiscRxn_k30, MiscRxn_k31,
...
static rateprop_ get_MiscRxn_rateprop_(chemistry_data_storage* my_rates, int i)
{
  switch(i) {
    ...
    case MiscRxn_k25: return MKPROP_SCALAR_(my_rates, k25);
    case MiscRxn_k26: return MKPROP_SCALAR_(my_rates, k26);
    // Radiative rates for 9-species
    case MiscRxn_k27: return MKPROP_SCALAR_(my_rates, k27);
    case MiscRxn_k28: return MKPROP_SCALAR_(my_rates, k28);
    case MiscRxn_k29: return MKPROP_SCALAR_(my_rates, k29);
    case MiscRxn_k30: return MKPROP_SCALAR_(my_rates, k30);
    case MiscRxn_k31: return MKPROP_SCALAR_(my_rates, k31);
    default: {
      rateprop_ out = {NULL, NULL};
      return out;
    }
...
struct rateprop_set_ {int id_offset; int len; fetch_rateprop_fn *fn;};
...
struct ratequery_rslt_ {grunstable_rateid_type rate_id; rateprop_ prop; };
...
static struct ratequery_rslt_ query_rateprop_(
  chemistry_data_storage* my_rates, long long i, bool use_rate_id
) {
...
grunstable_rateid_type grunstable_ratequery_id(const char* name) {
  if (name == NULL) { return UNDEFINED_RATE_ID_; }

  for (int set_idx = 0; set_idx < rate_registry_.len; set_idx++) {
    const struct rateprop_set_ cur_set = rate_registry_.sets[set_idx];
    for (int i = 0; i < cur_set.len; i++) {
      rateprop_ prop = cur_set.fn(NULL, i);
      if (strcmp(name, prop.name) == 0) { return cur_set.id_offset + i; }
    }
  }
...
const char* grunstable_ith_rate(
  ...
  if (out_rate_id != NULL) { *out_rate_id = tmp.rate_id; }
