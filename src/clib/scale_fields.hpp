...
inline void scale_fields_table(grackle_field_data* my_fields, double factor) {
  ...
  const int grid_start[3] = {
    my_fields->grid_start[0], my_fields->grid_start[1], my_fields->grid_start[2]
  };
  const int grid_end[3] = {
    my_fields->grid_end[0], my_fields->grid_end[1], my_fields->grid_end[2]
  };
...
inline void scale_fields_dust(
  chemistry_data* my_chemistry, grackle_field_data* my_fields, int imetal,
  gr_float factor
) {
