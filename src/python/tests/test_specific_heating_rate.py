...
def container_setup(density=mass_hydrogen_cgs,
    ...
    fc = setup_fluid_container(my_chemistry,
                               ...
                               converge=True)
    fc["specific_heating_rate"][:] = 1.0 # erg/s/g
    return fc
...
def get_heating_rate(fc):
    ...
    cooling_rate = fc.chemistry_data.cooling_units * fc["internal_energy"] / \
...
def test_specific_heating_rate_density():
    """
    For a constant specific heating rate [erg/s/g]
    => net cooling rate [erg*cm^3/s] should go as 1 / rho.
    """
    ...
    for i in range(6):
        ...
        assert_almost_equal(new_heating_rate/heating_rate,
                            density/new_density, decimal=1,
                            err_msg="\nViolates density constraint!")

        density = new_density
        heating_rate = new_heating_rate

def test_specific_heating_rate_density_units():
    """
    For a constant specific heating rate [erg/s/g]
...
def test_specific_heating_rate_redshift():
    """
    For a constant specific heating rate [erg/s/g]
    => net cooling rate [erg*cm^3/s] should be
...
def test_specific_heating_rate_a_units():
    """
    For a constant specific heating rate [erg/s/g]
    => net cooling rate [erg*cm^3/s] should be
    ...
    for i in range(6):
        ...
        fc["internal_energy"] *= (velocity_units / new_velocity_units)**2
